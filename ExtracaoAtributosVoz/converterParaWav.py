...
agente_voz = Agent(
    ...
    backstory=(
        ...
        "elas influenciam métricas como F0, HNR e jitter."
    ),
    llm="models/gemini-pro-latest",
    verbose=True
)
...
# ================================
# 🔹 ENVIO AO AGENTE COM PROMPT UNIVERSAL
# ================================
if __name__ == "__main__":
    ...
    # ================================
    # 🔹 PROMPT UNIVERSAL FOCADO EM PARKINSON
    # ================================
    prompt = f"""
Você é um especialista em processamento de fala, fonética clínica e análise acústica aplicada à detecção precoce de Doença de Parkinson (DP).  
Sua função é interpretar atributos acústicos extraídos automaticamente e compará-los com padrões cientificamente documentados de alterações vocais associadas à DP.

=== PERFIL DO FALANTE ===
Idade: {perfil_falante['idade']}
Sexo: {perfil_falante['sexo']}
Fumante: {perfil_falante['tabagista']}
Histórico de Doença Neurológica: {perfil_falante['condicoes_medicas']}

Use essas características para calibrar a análise — idade e sexo influenciam F0, formantes, jitter e HNR.

=== DADOS ACÚSTICOS EXTRAÍDOS ===
{json.dumps(features, indent=4)}

=== ORIENTAÇÕES DA ANÁLISE ===
1. Compare cada métrica com bancos de dados e literatura científica sobre marcadores de Parkinson.
2. Considere principalmente:
   - Aumento de jitter e shimmer
   - Redução ou instabilidade da F0
   - HNR reduzido (mais soprosidade/aspereza)
   - Alterações em F1–F4 devido à rigidez articulatória
   - Redução de variação prosódica
   - Alterações em MFCC compatíveis com disartria hipocinética
3. Gere uma análise técnico-clínica detalhada.
4. Estime risco de DP, mas sem diagnosticar (somente probabilidade).
5. Justifique todas as conclusões usando explicitamente os valores fornecidos.
"""
